...
void MapAdapter::run(Environment* env) {
  auto start_time = std::chrono::steady_clock::now();
  // 生成语义地图
  ParkingMap* parking_map = env->getParkingMapPtr();
  updateMap(env, parking_map);
...
void MapAdapter::GetSimulationDataFromStatic(Environment* env) {
  ...
  graph_ = GetGraphFromSimulationData(route);
...
common::WaypointsGraph MapAdapter::GetGraphFromSimulationData(
    const std::unordered_map<std::string, Route>& routes) {
  common::WaypointsGraph graph;

  // 为每个路线添加航路点
  for (const auto& route : routes) {
    graph.add_waypoint_list(route.second.points);
  }

  // 连接指定的航路点
  graph.connect(routes.at("C2").points[3], routes.at("R2L").points[0]);
  graph.connect(routes.at("C2").points[7], routes.at("R3L").points[0]);

  graph.connect(routes.at("C2").points[3], routes.at("R2R").points.back());
  graph.connect(routes.at("C2").points[7], routes.at("R3R").points.back());

  graph.connect(routes.at("R1L").points[0], routes.at("R1R").points.back());

  // Connect corners
  graph.connect(routes.at("C1").points[0], routes.at("R1C1BR").points[0]);
  graph.connect(routes.at("R1C1BR").points.back(),
                routes.at("R1L").points.back());
  graph.connect(routes.at("C1").points[2], routes.at("R2C1TR").points[0]);
  graph.connect(routes.at("R2C1TR").points.back(),
                routes.at("R2L").points.back());
  graph.connect(routes.at("C1").points[4], routes.at("R2C1BR").points[0]);
  graph.connect(routes.at("R2C1BR").points.back(),
                routes.at("R2L").points.back());
  graph.connect(routes.at("C1").points[6], routes.at("R3C1TR").points[0]);
  graph.connect(routes.at("R3C1TR").points.back(),
                routes.at("R3L").points.back());
  graph.connect(routes.at("C1").points[8], routes.at("R3C1BR").points[0]);
  graph.connect(routes.at("R3C1BR").points.back(),
                routes.at("R3L").points.back());
  graph.connect(routes.at("C1").points[10], routes.at("R4C1TR").points[0]);
  graph.connect(routes.at("R4C1TR").points.back(),
                routes.at("R4L").points.back());
  graph.connect(routes.at("C1").points[12], routes.at("R4C1BR").points[0]);
  graph.connect(routes.at("R4C1BR").points.back(),
                routes.at("R4L").points.back());

  graph.connect(routes.at("C2").points[0], routes.at("R1C2BL").points[0]);
  graph.connect(routes.at("R1C2BL").points.back(), routes.at("R1L").points[2]);
  graph.connect(routes.at("C2").points[2], routes.at("R2C2TL").points[0]);
  graph.connect(routes.at("R2C2TL").points.back(), routes.at("R2L").points[0]);
  graph.connect(routes.at("C2").points[4], routes.at("R2C2BL").points[0]);
  graph.connect(routes.at("R2C2BL").points.back(), routes.at("R2L").points[0]);
  graph.connect(routes.at("C2").points[6], routes.at("R3C2TL").points[0]);
  graph.connect(routes.at("R3C2TL").points.back(), routes.at("R3L").points[0]);
  graph.connect(routes.at("C2").points[8], routes.at("R3C2BL").points[0]);
  graph.connect(routes.at("R3C2BL").points.back(), routes.at("R3L").points[0]);
  graph.connect(routes.at("C2").points[10], routes.at("R4C2TL").points[0]);
  graph.connect(routes.at("R4C2TL").points.back(), routes.at("R4L").points[0]);
  graph.connect(routes.at("C2").points[12], routes.at("R4C2BL").points[0]);
  graph.connect(routes.at("R4C2BL").points.back(), routes.at("R4L").points[0]);

  graph.connect(routes.at("C2").points[0], routes.at("R1C2BR").points[0]);
  graph.connect(routes.at("R1C2BR").points.back(),
                routes.at("R1R").points.back());
  graph.connect(routes.at("C2").points[2], routes.at("R2C2TR").points[0]);
  graph.connect(routes.at("R2C2TR").points.back(),
                routes.at("R2R").points.back());
  graph.connect(routes.at("C2").points[4], routes.at("R2C2BR").points[0]);
  graph.connect(routes.at("R2C2BR").points.back(),
                routes.at("R2R").points.back());
  graph.connect(routes.at("C2").points[6], routes.at("R3C2TR").points[0]);
  graph.connect(routes.at("R3C2TR").points.back(),
                routes.at("R3R").points.back());
  graph.connect(routes.at("C2").points[8], routes.at("R3C2BR").points[0]);
  graph.connect(routes.at("R3C2BR").points.back(),
                routes.at("R3R").points.back());
  graph.connect(routes.at("C2").points[10], routes.at("R4C2TR").points[0]);
  graph.connect(routes.at("R4C2TR").points.back(),
                routes.at("R4R").points.back());
  graph.connect(routes.at("C2").points[12], routes.at("R4C2BR").points[0]);
  graph.connect(routes.at("R4C2BR").points.back(),
                routes.at("R4R").points.back());

  graph.connect(routes.at("R4L").points[0], routes.at("R4R").points.back());

  graph.connect(routes.at("EXT").points[1], routes.at("EXTL").points[0]);
  graph.connect(routes.at("EXT").points[1], routes.at("EXTR").points[0]);
  graph.connect(routes.at("EXTL").points[0], routes.at("R1L").points.back());
  graph.connect(routes.at("EXTR").points[0],
                routes.at("R1L").points[routes.at("R1L").points.size() - 5]);

  // 打印图信息

  // 定义起点和终点
  std::shared_ptr<common::Vertex> start = graph.search({80.45, 64.95});
  std::shared_ptr<common::Vertex> goal = graph.search({3.07, 4.5});
